...
void leValidaPessoaMaisVelha ( int idade[], int qtde){
	...
	printf("\nA pessoa mais velha tem a idade de %i anos. \n", maiorIdade);//Como fazer para achar a posição das idades mais velha no vetor?
	for(i = 0; i <= qtde; i++){
		if(maiorIdade == idade[i] ){
			posicao[count] = i;//Pegando a posição das idades mais velhas
			count++;		
		}
	}
	...
	for(i = 0; i < count; i++){
		i == 0 ?  printf(" %i", posicao[i]) : printf(" - %i", posicao[i])  ;// esse é o if ternário.
	}
